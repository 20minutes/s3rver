...
function parseConfigureBucket(bucketName, memo = []) {
  ...
  const bucketConfigs = []
  while (idx < program.rawArgs.length && !program.rawArgs[idx].startsWith('-')) {
...
program
  ...
  .option('-a, --address <value>', 'Hostname or IP to bind to', S3rver.defaultOptions.address)
  .option('-p, --port <n>', 'Port of the http server', S3rver.defaultOptions.port)
  .option('-s, --silent', 'Suppress log messages', S3rver.defaultOptions.silent)
  .option('--key <path>', 'Path to private key file for running with TLS', fs.readFileSync)
  .option('--cert <path>', 'Path to certificate file for running with TLS', fs.readFileSync)
  .option(
...
program.options.find((option) => option.is('--configure-bucket')).variadic = false

program.on('--help', () => {
  console.info('')
  console.info('Examples:')
  console.info('  $ s3rver -d /tmp/s3rver -a 0.0.0.0 -p 0')
  console.info('  $ s3rver -d /tmp/s3rver --configure-bucket test-bucket ./cors.xml ./website.xml')
})
