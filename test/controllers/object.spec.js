...
describe('Operations on Objects', () => {
  ...
  describe('Delete Multiple Objects', () => {
    it('deletes an image from a bucket', async () => {
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'large',
    ...
    it('deletes nonexistent objects', async () => {
  ...
  describe('GET Object', () => {
    it('stores a large buffer in a bucket', async () => {
      const data = await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'large',
          Body: Buffer.alloc(20 * 1024 ** 2),
        })
        .promise()
      expect(data.ETag).to.match(/"[a-fA-F0-9]{32}"/)
    })

    it('gets an image from a bucket', async () => {
      ...
      await s3Client
        ...
        .promise()
      const object = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'image' }).promise()
    ...
    it('can HEAD an empty object in a bucket', async () => {
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'somekey',
        ...
        .promise()
      const object = await s3Client.headObject({ Bucket: 'bucket-a', Key: 'somekey' }).promise()
      expect(object.ETag).to.match(/"[a-fA-F0-9]{32}"/)
    })

    it('gets partial image from a bucket with a range request', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('gets a response without range headers when no range is specified in the request', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('gets a response with range headers when the requested range starts on byte 0 and no end', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('returns 416 error for out of bounds range requests', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
    ...
    it('returns actual length of data for partial out of bounds range requests', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      const { size: filesize } = fs.statSync(file)
      await s3Client
        .putObject({
    ...
    it('finds a text file in a multi directory path', async () => {
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('returns image metadata from a bucket in HEAD request', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      ...
      await s3Client
        ...
        .promise()
      const object = await s3Client.headObject({ Bucket: 'bucket-a', Key: 'image' }).promise()
      expect(object.ETag).to.equal(JSON.stringify(md5(fileContent)))
      expect(object.ContentLength).to.equal(fileContent.length)
      expect(object.ContentType).to.equal('image/jpeg')
    })

    it('fails to find an image from a bucket', async () => {
      let error
      try {
        await s3Client.getObject({ Bucket: 'bucket-a', Key: 'image' }).promise()
      } catch (err) {
        error = err
  ...
  describe('GET Object ACL', () => {
    it('returns a dummy acl for an object', async () => {
      const object = await s3Client.getObjectAcl({ Bucket: 'bucket-a', Key: 'image0' }).promise()
      expect(object.Owner.DisplayName).to.equal('S3rver')
    })
  })

  describe('GET Object tagging', () => {
    it("errors when getting tags for an object that doesn't exist", async () => {
      await expect(
        s3Client
          .getObjectTagging({
            Bucket: 'bucket-a',
  ...
  describe('POST Object', () => {
    it('stores a text object for a multipart/form-data request', async () => {
      ...
      const object = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'text' }).promise()
      expect(object.ContentType).to.equal('binary/octet-stream')
      expect(object.Body).to.deep.equal(Buffer.from('Hello!'))
    })

    it('rejects requests with an invalid content-type', async () => {
      let res
      try {
        res = await axios({
          url: 'bucket-a',
    ...
    it('stores a text object without filename part metadata', async () => {
      const form = new FormData()
      form.append('key', 'text')
    ...
    it('stores a text object with a content-type', async () => {
      const form = new FormData()
      form.append('key', 'text')
    ...
    it('returns the location of the stored object in a header', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      const form = new FormData()
      ...
      const objectRes = await axios(res.headers.location, {
        responseType: 'arraybuffer',
      })
      expect(objectRes.data).to.deep.equal(fs.readFileSync(file))
    })

    it('returns the location of the stored object in a header with vhost URL', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      const form = new FormData()
      form.append('key', 'image')
      form.append('file', fs.createReadStream(file))
      ...
      expect(res.status).to.equal(204)
      expect(res.headers).to.have.property('location', new URL('/image', `http://bucket-a`).href)
    })

    it('returns the location of the stored object in a header with subdomain URL', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      const form = new FormData()
      form.append('key', 'image')
      form.append('file', fs.createReadStream(file))
    ...
    it('returns a 200 status code with empty response body', async () => {
      const form = new FormData()
      form.append('key', 'text')
    ...
    it('returns a 201 status code with XML response body', async () => {
      const form = new FormData()
      form.append('key', 'text')
      ...
      expect(res.data).to.contain('<PostResponse>')
      expect(res.data).to.contain('<Bucket>bucket-a</Bucket><Key>text</Key>')
    })

    it('returns a 204 status code when an invalid status is specified', async () => {
      const form = new FormData()
      form.append('key', 'text')
    ...
    it('redirects a custom location with search parameters', async () => {
      ...
      expect(location.searchParams.get('etag')).to.equal(JSON.stringify(md5('Hello!')))
    })

    it('redirects a custom location using deprecated redirect fieldname', async () => {
      const successRedirect = new URL('http://foo.local/path?bar=baz')
      const form = new FormData()
      form.append('key', 'text')
    ...
    it('ignores deprecated redirect field when success_action_redirect is specified', async () => {
      const successRedirect = new URL('http://foo.local/path?bar=baz')
      const form = new FormData()
      form.append('key', 'text')
      form.append('success_action_redirect', successRedirect.href)
    ...
    it('ignores status field when redirect is specified', async () => {
      const successRedirect = new URL('http://foo.local/path?bar=baz')
      const form = new FormData()
      form.append('key', 'text')
      form.append('success_action_redirect', successRedirect.href)
    ...
    it('ignores fields specified after the file field', async () => {
      const form = new FormData()
      form.append('key', 'text')
      form.append('file', 'Hello!')
      form.append('Content-Type', 'text/plain')
      ...
      const objectRes = await axios(res.headers.location)
      expect(res.status).to.equal(204)
      expect(objectRes.headers).to.not.have.property('content-type', 'text/plain')
    })

    it('rejects requests with no key field', async () => {
      const form = new FormData()
      form.append('file', 'Hello!')
      let res
      try {
      ...
      expect(res.data).to.contain('<ArgumentName>key</ArgumentName><ArgumentValue></ArgumentValue>')
    })

    it('rejects requests with zero-length key', async () => {
      const form = new FormData()
      form.append('key', '')
      form.append('file', 'Hello!')
      let res
      ...
      expect(res.status).to.equal(400)
      expect(res.data).to.contain('<Message>User key must have a length greater than 0.</Message>')
    })
  ...
  describe('PUT Object', () => {
    it('stores a text object in a bucket', async () => {
      const data = await s3Client
    ...
    it('stores a different image and update the previous image', async () => {
      const files = [
      ...
      const newObject = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'image' }).promise()
    ...
    it('distinguishes keys stored with and without a trailing /', async () => {
      await s3Client.putObject({ Bucket: 'bucket-a', Key: 'text', Body: 'Hello!' }).promise()
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
        ...
        .promise()
      const obj1 = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'text' }).promise()
      const obj2 = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'text/' }).promise()
      expect(obj1.Body.toString()).to.equal('Hello!')
      expect(obj2.Body.toString()).to.equal('Goodbye!')
    })

    it('stores a text object with invalid win32 path characters and retrieves it', async () => {
      const reservedChars = '\\/:*?"<>|'
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('stores a text object with no content type and retrieves it', async () => {
      const res = await axios({
        method: 'put',
        url: 'bucket-a/text',
        baseURL: s3Client.endpoint.href,
      ...
      expect(res.status).to.equal(200)
      const data = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'text' }).promise()
      expect(data.ContentType).to.equal('binary/octet-stream')
    })

    it('stores a text object with some custom metadata', async () => {
      const data = await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'textmetadata',
        ...
        .promise()
      expect(data.ETag).to.match(/"[a-fA-F0-9]{32}"/)
      const object = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'textmetadata' }).promise()
      expect(object.Metadata.somekey).to.equal('value')
    })

    it('stores an image in a bucket', async () => {
      const file = require.resolve('../fixtures/image0.jpg')
      const data = await s3Client
        .putObject({
          Bucket: 'bucket-a',
    ...
    it('stores a file in bucket with gzip encoding', async () => {
      ...
      const object = await s3Client.getObject({ Bucket: 'bucket-a', Key: 'jquery' }).promise()
      expect(object.ContentEncoding).to.equal('gzip')
      expect(object.ContentType).to.equal('application/javascript')
    })

    it('stores and retrieves an object while mounted on a subpath', async () => {
      const app = express()
    ...
    it('stores an object in a bucket after all objects are deleted', async () => {
      const bucket = 'foobars'
      await s3Client.createBucket({ Bucket: bucket }).promise()
      await s3Client.putObject({ Bucket: bucket, Key: 'foo.txt', Body: 'Hello!' }).promise()
      await s3Client.deleteObject({ Bucket: bucket, Key: 'foo.txt' }).promise()
      await s3Client.putObject({ Bucket: bucket, Key: 'foo2.txt', Body: 'Hello2!' }).promise()
    })

    it('stores an object with a storage class', async () => {
      await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'somekey',
    ...
    it('fails to store an object with an invalid storage class', async () => {
      let error
      try {
        await s3Client
          .putObject({
    ...
    describe('Chunked Upload', () => {
      ...
      it('stores an object using chunked transfer encoding', async () => {
        const chunks = [Buffer.alloc(8192), 'Hello!', '']
        const signer = createSigner({ Bucket: 'bucket-a', Key: 'text' }, chunks)
      ...
      it('fails to store an object when an initial chunk is smaller than 8KB', async () => {
        ...
        expect(resBody.Error).to.include({
          ...
          Message: 'Only the last chunk is allowed to have a size less than 8192 bytes',
          Chunk: 3,
          BadChunkSize: chunks[1].length,
        })
      })

      it('fails to store an object when a chunked transfer terminates with a non-empty chunk', async () => {
        const chunks = ['Hello!']
        const signer = createSigner({ Bucket: 'bucket-a', Key: 'text' }, chunks)
        const opts = signer.sign()
        const req = http.request(opts)
      ...
      it('fails to store an object when no decoded content length is provided', async () => {
        const chunks = ['Hello!', '']
        const signer = createSigner({ Bucket: 'bucket-a', Key: 'text' }, chunks)
      ...
      it('fails to store an object when the decoded content length does not match', async () => {
        const chunks = ['Hello!', '']
        const signer = createSigner({ Bucket: 'bucket-a', Key: 'text' }, chunks)
  ...
  describe('PUT Object - Copy', () => {
    it('copies an image object into another bucket', async () => {
      const srcKey = 'image'
    ...
    it('copies an image object into another bucket including its metadata', async () => {
      const srcKey = 'image'
      const destKey = 'image/jamie'
      ...
      await s3Client
        ...
        .promise()
      const object = await s3Client.getObject({ Bucket: 'bucket-b', Key: destKey }).promise()
      expect(object.Metadata).to.have.property('somekey', 'value')
      expect(object.ContentType).to.equal('image/jpeg')
      expect(object.ETag).to.equal(data.ETag)
    })

    it('copies an object using spaces/unicode chars in keys', async () => {
    ...
    it('updates the metadata of an image object', async () => {
      const srcKey = 'image'
      const destKey = 'image/jamie'
    ...
    it('fails to update the metadata of an image object when no REPLACE MetadataDirective is specified', async () => {
      const key = 'image'
    ...
    it('fails to copy an image object because the object does not exist', async () => {
      let error
      try {
        await s3Client
          .copyObject({
    ...
    it('fails to copy an image object because the source bucket does not exist', async () => {
      let error
      try {
        await s3Client
          .copyObject({
  ...
  describe('PUT Object tagging', () => {
    it('tags an object in a bucket', async () => {
      await s3Client.putObject({ Bucket: 'bucket-a', Key: 'text', Body: 'Hello!' }).promise()
    ...
    it("errors when tagging an object that doesn't exist", async () => {
      await expect(
        s3Client
          .putObjectTagging({
            Bucket: 'bucket-a',
  ...
  describe('Initiate/Upload/Complete Multipart upload', () => {
    it('uploads a text file to a multi directory path', async () => {
      const data = await s3Client
        .putObject({
          Bucket: 'bucket-a',
          Key: 'multi/directory/path/text',
    ...
    it('completes a managed upload <=5MB', async () => {
      const data = await s3Client
        .upload({
          Bucket: 'bucket-a',
          Key: 'multi/directory/path/multipart',
          Body: Buffer.alloc(2 * 1024 ** 2), // 2MB
        })
        .promise()
      expect(data.ETag).to.match(/"[a-fA-F0-9]{32}"/)
    })

    it('completes a managed upload >5MB (multipart upload)', async () => {
      const data = await s3Client
        .upload({
          Bucket: 'bucket-a',
          Key: 'multi/directory/path/multipart',
          Body: Buffer.alloc(20 * 1024 ** 2), // 20MB
        })
        .promise()
      expect(data.ETag).to.match(/"[a-fA-F0-9]{32}"/)
    })

    it('completes a multipart upload with unquoted ETags', async () => {
      const data = await s3Client
    ...
    it('completes a multipart upload with metadata', async () => {
      const data = await s3Client
        .upload({
          Bucket: 'bucket-a',
          Key: 'multi/directory/path/multipart',
    ...
    it('should upload a part by copying it', async () => {
    ...
    it('should copy parts from bucket to bucket', async () => {
      const upload = await s3Client
        .createMultipartUpload({
          Bucket: 'bucket-a',
          Key: 'merged',
    ...
    it('should copy a part range from bucket to bucket', async () => {
      const upload = await s3Client
        .createMultipartUpload({
          Bucket: 'bucket-a',
          Key: 'merged',
    ...
    it('fails to copy a part range for an out of bounds requests', async () => {
      const upload = await s3Client
        .createMultipartUpload({
          Bucket: 'bucket-a',
          Key: 'merged',
        ...
        .promise()
      const body = Buffer.alloc(20 * 1024 ** 2) // 20MB
      await s3Client
        .putObject({
          Bucket: 'bucket-b',
          Key: 'part',
    ...
    it('fails to copy a part from a nonexistent bucket', async () => {
      const upload = await s3Client
        .createMultipartUpload({
          Bucket: 'bucket-a',
          Key: 'merged',
    ...
    it('fails to copy a part from a nonexistent key', async () => {
      const upload = await s3Client
        .createMultipartUpload({
          Bucket: 'bucket-a',
          Key: 'merged',
