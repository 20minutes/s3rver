...
describe('RoutingRule', () => {
  ...
  describe('Redirect', () => {
    ...
    it('redirects using only HostName', () => {
      ...
      expect(rule.getRedirectLocation('key', defaults)).to.equal('https://localhost/key')
    })
    ...
    it('redirects using only HttpRedirectCode', () => {
      ...
      expect(rule.getRedirectLocation('key', defaults)).to.equal('https://example.com/key')
    })
    ...
    it('redirects using only Protocol', () => {
      ...
      expect(rule.statusCode).to.equal(301)
      expect(rule.getRedirectLocation('key', defaults)).to.equal('http://example.com/key')
    })
    ...
    it('redirects using only ReplaceKeyWith', () => {
      ...
      expect(rule.statusCode).to.equal(301)
      expect(rule.getRedirectLocation('key', defaults)).to.equal('https://example.com/replacement')
    })
